...
from app.repositories import worker_skill as eskill_repo
...
class TestDepartmentRepository:
    ...
    def test_crud_full_flow(self):
        ...
        assert dept["name"] == "宸ョ▼閮?
        assert dept["description"] == "鐮斿彂閮ㄩ棬"
        assert dept["manager"] == "寮犵粡鐞?

        # Read by id
        fetched = dept_repo.get_department_by_id(dept["id"])
        assert fetched is not None
        assert fetched["name"] == "宸ョ▼閮?

        # Read by name
        by_name = dept_repo.get_department_by_name("宸ョ▼閮?)
        ...
        # Update
        updated = dept_repo.update_department(dept["id"], {"name": "鎶€鏈儴", "manager": "鏉庣粡鐞?})
        assert updated is not None
        assert updated["name"] == "鎶€鏈儴"
        assert updated["manager"] == "鏉庣粡鐞?
        assert updated["description"] == "鐮斿彂閮ㄩ棬"  # unchanged
    ...
    def test_get_by_name_returns_none_for_nonexistent(self):
        assert dept_repo.get_department_by_name("涓嶅瓨鍦ㄧ殑閮ㄩ棬") is None
    ...
    def test_update_partial_fields_ignores_none(self):
        dept = dept_repo.create_department({"name": "璐㈠姟閮?, "description": "璐㈠姟", "manager": "鐜嬬粡鐞?})
        updated = dept_repo.update_department(dept["id"], {"name": "瀹¤閮?, "description": None})
        assert updated["name"] == "瀹¤閮?
        assert updated["description"] == "璐㈠姟"  # None was ignored
...
class TestWorkerRepository:
    def test_get_all_returns_empty_when_no_data(self):
        assert emp_repo.get_all_workers() == []

    def test_crud_full_flow(self):
        emp = emp_repo.create_worker(
            {
                "name": "寮犱笁",
                "department_id": 1,
                "salary": 15000.0,
        ...
        # Read by id
        fetched = emp_repo.get_worker_by_id(emp["id"])
        assert fetched is not None
        assert fetched["name"] == "寮犱笁"

        # Read all
        all_emps = emp_repo.get_all_workers()
        assert len(all_emps) == 1

        # Update
        updated = emp_repo.update_worker(emp["id"], {"name": "鏉庡洓", "salary": 18000.0})
        assert updated["name"] == "鏉庡洓"
        assert updated["salary"] == 18000.0
        assert updated["department_id"] == 1  # unchanged

        # Delete
        assert emp_repo.delete_worker(emp["id"]) is True
        assert emp_repo.get_worker_by_id(emp["id"]) is None

    def test_get_by_id_returns_none_for_nonexistent(self):
        assert emp_repo.get_worker_by_id(9999) is None

    def test_update_returns_none_for_nonexistent(self):
        assert emp_repo.update_worker(9999, {"name": "x"}) is None

    def test_delete_returns_false_for_nonexistent(self):
        assert emp_repo.delete_worker(9999) is False

    def test_get_workers_by_department(self):
        emp_repo.create_worker({"name": "A", "department_id": 1, "salary": 10000.0})
        emp_repo.create_worker({"name": "B", "department_id": 1, "salary": 12000.0})
        emp_repo.create_worker({"name": "C", "department_id": 2, "salary": 11000.0})

        dept1 = emp_repo.get_workers_by_department(1)
        dept2 = emp_repo.get_workers_by_department(2)
        assert len(dept1) == 2
        assert len(dept2) == 1
        assert emp_repo.get_workers_by_department(999) == []

    def test_update_partial_fields_ignores_none(self):
        emp = emp_repo.create_worker({"name": "鐜嬩簲", "department_id": 1, "salary": 10000.0})
        updated = emp_repo.update_worker(emp["id"], {"name": "璧靛叚", "department_id": None})
        assert updated["name"] == "璧靛叚"
...
class TestAttendanceRepository:
    ...
    def test_calculate_status_late_and_early_leave(self):
        # Both late and early 鈥?the function returns "late" in this case
    ...
    def test_crud_full_flow(self):
        ...
        # Delete (via employee list 鈥?no dedicated delete in attendance repo, test via get)
...
class TestLeaveRepository:
    ...
    def test_crud_full_flow(self):
        ...
        # Update 鈥?leave uses setattr for ALL fields (even None)
        updated = leave_repo.update_leave(leave["id"], {"status": "approved", "approver": "鏉庣粡鐞?})
        assert updated["status"] == "approved"
        assert updated["approver"] == "鏉庣粡鐞?

        # Delete 鈥?no delete function in leave repo, verify update
        assert updated is not None
...
class TestPayrollRepository:
    ...
    def test_crud_full_flow(self):
        ...
        # Update 鈥?payroll uses setattr for ALL fields
...
class TestProjectRepository:
    ...
    def test_crud_full_flow(self):
        ...
        # Update
        updated = proj_repo.update_project(proj["id"], {"status": "active", "description": "鍗囩骇涓?})
        assert updated["status"] == "active"
        assert updated["description"] == "鍗囩骇涓?
        assert updated["name"] == "HR绯荤粺V2"  # unchanged
...
class TestProjectMemberRepository:
    ...
    def test_crud_full_flow(self):
        ...
        # Update
        updated = proj_repo.update_member(member["id"], {"role": "鎶€鏈礋璐ｄ汉"})
        assert updated["role"] == "鎶€鏈礋璐ｄ汉"
    ...
    def test_update_partial_ignores_none(self):
        ...
        updated = proj_repo.update_member(member["id"], {"role": "娴嬭瘯", "assigned_date": None})
        assert updated["role"] == "娴嬭瘯"
...
class TestProjectTimesheetRepository:
    ...
    def test_crud_full_flow(self):
        ...
        assert fetched is not None
        assert fetched["description"] == "寮€鍙戝姛鑳?
    ...
    def test_update_partial_ignores_none(self):
        ...
        assert updated["hours"] == 6.0
        assert updated["description"] == "鍘熷鎻忚堪"  # None ignored
...
class TestSkillCatalogRepository:
    ...
    def test_crud_full_flow(self):
        ...
        # Update
        updated = catalog_repo.update_skill(skill["id"], {"description": "閫氱敤缂栫▼璇█", "category": "寮€鍙?})
        assert updated["description"] == "閫氱敤缂栫▼璇█"
    ...
    def test_get_by_name_returns_none_for_nonexistent(self):
        assert catalog_repo.get_skill_by_name("涓嶅瓨鍦ㄧ殑鎶€鑳?) is None
    ...
    def test_get_all_filters_category(self):
        catalog_repo.create_skill({"name": "Python", "category": "缂栫▼", "created_at": self.NOW})
        catalog_repo.create_skill({"name": "娌熼€?, "category": "杞妧鑳?, "created_at": self.NOW})
        result = catalog_repo.get_all_skills(category="缂栫▼")
        assert len(result) == 1
        assert result[0]["name"] == "Python"

    def test_count_employee_skills_by_skill_id(self):
        skill = catalog_repo.create_skill({"name": "Python", "category": "缂栫▼", "created_at": self.NOW})
        eskill_repo.create_skill(
            {
                "employee_id": 1,
                "skill_name": "Python",
    ...
    def test_count_project_requirements_by_skill_id(self):
        skill = catalog_repo.create_skill({"name": "Go", "category": "缂栫▼", "created_at": self.NOW})
    ...
    def test_update_partial_ignores_none(self):
        ...
        updated = catalog_repo.update_skill(skill["id"], {"description": "瀹夊叏璇█", "category": None})
        assert updated["description"] == "瀹夊叏璇█"
        assert updated["category"] == "缂栫▼"  # None ignored
...
class TestAgentMemoryRepository:
    ...
    # 鈹€鈹€ Memory CRUD 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
    ...
    def test_crud_full_flow(self):
        ...
        assert mem["subject"] == "娴嬭瘯涓婚"

        # Read by id
        fetched = memory_repo.get_memory_by_id(mem["id"])
        assert fetched is not None
        assert fetched["content"] == "娴嬭瘯鍐呭"

        # Update
        updated = memory_repo.update_memory(mem["id"], {"content": "鏇存柊鍐呭", "importance": 5})
        assert updated["content"] == "鏇存柊鍐呭"
    ...
    # 鈹€鈹€ Memory queries 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€

    def test_get_memories_by_user_tag(self):
        memory_repo.create_memory(self._make_memory_data(user_tag="user-1"))
        memory_repo.create_memory(self._make_memory_data(user_tag="user-1", subject="涓婚2"))
    ...
    def test_get_memories_by_subject(self):
        memory_repo.create_memory(self._make_memory_data(subject="钖祫"))
        memory_repo.create_memory(self._make_memory_data(subject="钖祫", user_tag="user-2"))
        memory_repo.create_memory(self._make_memory_data(subject="鑰冨嫟"))
        result = memory_repo.get_memories_by_subject("钖祫")
        assert len(result) == 2

    def test_get_memories_by_subject_active_only(self):
        memory_repo.create_memory(self._make_memory_data(subject="钖祫", is_active=True))
        memory_repo.create_memory(self._make_memory_data(subject="钖祫", is_active=False))
        result = memory_repo.get_memories_by_subject("钖祫", active_only=True)
        assert len(result) == 1
        result_all = memory_repo.get_memories_by_subject("钖祫", active_only=False)
        assert len(result_all) == 2

    def test_search_memories_by_content_ilike(self):
        memory_repo.create_memory(self._make_memory_data(content="Python缂栫▼璇█"))
        memory_repo.create_memory(self._make_memory_data(content="Java寮€鍙戞鏋?))
        memory_repo.create_memory(self._make_memory_data(content="python鏁版嵁鍒嗘瀽"))  # case-insensitive
    ...
    def test_get_preference_by_user_tag_and_subject(self):
        ...
        result = memory_repo.get_preference_by_user_tag_and_subject("user-1", "璇█鍋忓ソ")
        assert result is not None
        assert result["content"] == "涓枃"
        assert result["memory_type"] == "preference"

    def test_get_preference_returns_none_when_not_found(self):
        assert memory_repo.get_preference_by_user_tag_and_subject("user-1", "涓嶅瓨鍦?) is None

    # 鈹€鈹€ Deactivate 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
class TestConversationMessageRepository:
    ...
    def test_get_messages_by_session_ordered_by_created_at_asc(self):
        ...
        msgs = memory_repo.get_messages_by_session("sess-1")
        assert len(msgs) == 3
        assert msgs[0]["content"] == "娑堟伅0"
        assert msgs[2]["content"] == "娑堟伅2"
