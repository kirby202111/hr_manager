...
def get_memories_by_subject(subject: str, active_only: bool = True, db: Session | None = None) -> list[dict]:
    with db_session(db) as session:
...
def search_memories_by_content(
    user_tag: str, keyword: str, active_only: bool = True, db: Session | None = None
) -> list[dict]:
    with db_session(db) as session:
        query = session.query(MemoryORM).filter_by(user_tag=user_tag)
        if active_only:
...
def get_recent_memories(
    user_tag: str, limit: int = 20, active_only: bool = True, db: Session | None = None
) -> list[dict]:
    with db_session(db) as session:
        query = session.query(MemoryORM).filter_by(user_tag=user_tag)
        if active_only:
...
def get_important_memories(
    user_tag: str, min_importance: int = 4, active_only: bool = True, db: Session | None = None
) -> list[dict]:
    with db_session(db) as session:
        query = session.query(MemoryORM).filter_by(user_tag=user_tag)
        if active_only:
...
def deactivate_memory(memory_id: int, db: Session | None = None) -> bool:
    with db_session(db) as session:
        m = session.get(MemoryORM, memory_id)
        if m is None:
            return False
...
def deactivate_expired_memories(db: Session | None = None) -> int:
    with db_session(db) as session:
        count = (
            session.query(MemoryORM)
            .filter(
...
def get_pending_reminders(user_tag: str, before: datetime, db: Session | None = None) -> list[dict]:
    with db_session(db) as session:
        reminder_ids = (
...
def mark_reminder_triggered(reminder_id: int, db: Session | None = None) -> dict | None:
    with db_session(db) as session:
...
def delete_reminder(reminder_id: int, db: Session | None = None) -> bool:
    with db_session(db) as session:
        r = session.get(ReminderORM, reminder_id)
        if r is None:
            return False
...
def delete_messages_by_session(session_id: str, db: Session | None = None) -> bool:
    with db_session(db) as session:
...
def trim_session_messages(session_id: str, max_messages: int, db: Session | None = None) -> None:
    with db_session(db) as session:
