...
from app.models.qualification import WorkstationCertificationRequirement
...
def list_workstation_certification_requirements(
    workstation_id: int | None = None, db: Session | None = None
) -> list[dict]:
...
def get_workstation_certification_requirement_by_id(
    ...
    with db_session(db) as session:
        row = session.get(WorkstationCertificationRequirement, workstation_certification_requirement_id)
