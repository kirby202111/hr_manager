...
def update_project(project_id: int, project_data: dict) -> dict | None:
    with SessionLocal() as session:
        project = session.get(ProjectORM, project_id)
        if project is None:
            return None
...
def count_requirements(project_id: int) -> int:
    with SessionLocal() as session:
        return session.query(ReqORM).filter_by(project_id=project_id).count()


def count_members(project_id: int) -> int:
    with SessionLocal() as session:
...
def get_requirement_by_project_and_skill(project_id: int, skill_id: int) -> dict | None:
    with SessionLocal() as session:
        req = session.query(ReqORM).filter_by(project_id=project_id, skill_id=skill_id).first()
        return req.to_dict() if req else None


def create_requirement(req_data: dict) -> dict:
    with SessionLocal() as session:
        req = ReqORM(**req_data)
        session.add(req)
        session.commit()
...
def update_requirement(req_id: int, req_data: dict) -> dict | None:
    with SessionLocal() as session:
        req = session.get(ReqORM, req_id)
        if req is None:
            return None
...
def get_members_by_project(project_id: int) -> list[dict]:
    with SessionLocal() as session:
...
def get_member_by_employee_project(employee_id: int, project_id: int) -> dict | None:
    with SessionLocal() as session:
...
def update_member(member_id: int, member_data: dict) -> dict | None:
    with SessionLocal() as session:
        member = session.get(MemberORM, member_id)
        if member is None:
            return None
...
def get_timesheet_by_id(timesheet_id: int) -> dict | None:
    with SessionLocal() as session:
        ts = session.get(TimesheetORM, timesheet_id)
        return ts.to_dict() if ts else None


def create_timesheet(ts_data: dict) -> dict:
    with SessionLocal() as session:
...
def update_timesheet(timesheet_id: int, ts_data: dict) -> dict | None:
    with SessionLocal() as session:
        ts = session.get(TimesheetORM, timesheet_id)
        if ts is None:
            return None
...
def delete_timesheet(timesheet_id: int) -> bool:
    with SessionLocal() as session:
        ts = session.get(TimesheetORM, timesheet_id)
        if ts is None:
            return False
...
def get_progress_by_project(project_id: int) -> dict:
    with SessionLocal() as session:
