...
from app.models.skill_definition import SkillDefinition as SkillDefinitionORM


def get_all_skills(category: str | None = None, db: Session | None = None) -> list[dict]:
    with db_session(db) as session:
        query = session.query(SkillDefinitionORM)
        if category:
...
def get_skill_by_id(skill_id: int, db: Session | None = None) -> dict | None:
    with db_session(db) as session:
        skill = session.get(SkillDefinitionORM, skill_id)
        return skill.to_dict() if skill else None


def get_skill_by_name(name: str, db: Session | None = None) -> dict | None:
    with db_session(db) as session:
        skill = session.query(SkillDefinitionORM).filter_by(name=name).first()
        return skill.to_dict() if skill else None


def create_skill(skill_data: dict, db: Session | None = None) -> dict:
    with db_session(db) as session:
        skill = SkillDefinitionORM(**skill_data)
        session.add(skill)
        session.flush()
...
def count_employee_skills_by_skill_id(skill_id: int, db: Session | None = None) -> int:
    from app.models.worker_skill import EmployeeSkill as EmployeeSkillORM
