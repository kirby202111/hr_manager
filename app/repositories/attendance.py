...
def get_attendance_by_worker_date(worker_id: int, record_date, db: Session | None = None) -> dict | None:
    with db_session(db) as session:
        record = session.query(AttendanceORM).filter_by(worker_id=worker_id, date=record_date).first()
        return record.to_dict() if record else None


def get_attendance_by_worker(worker_id: int, db: Session | None = None) -> list[dict]:
    with db_session(db) as session:
        return [record.to_dict() for record in session.query(AttendanceORM).filter_by(worker_id=worker_id).all()]
