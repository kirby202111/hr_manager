...
def _require_row(attendance_record_id: int, db: Session | None = None) -> dict:
    row = attendance_repo.get_attendance_record_by_id(attendance_record_id, db)
    if row is None:
...
def list_attendance_records(
    ...
    rows = attendance_repo.list_attendance_records(worker_id, work_date, status, db)
...
def create_attendance_record(data: AttendanceRecordCreate, db: Session | None = None) -> AttendanceRecordResponse:
    ...
    if (
        attendance_repo.get_attendance_record_by_worker_and_work_date(payload["worker_id"], payload["work_date"], db)
        is not None
    ):
        raise ConflictError("Attendance record already exists")
    row = attendance_repo.create_attendance_record(payload, db)
...
def update_attendance_record(
    ...
    _validate_payload(payload, db)
    existing = attendance_repo.get_attendance_record_by_worker_and_work_date(
        payload["worker_id"], payload["work_date"], db
    )
    if existing is not None and existing["id"] != attendance_record_id:
        raise ConflictError("Attendance record already exists")
    row = attendance_repo.update_attendance_record(attendance_record_id, data.model_dump(exclude_unset=True), db)
    if row is None:
        raise NotFoundError(f"Attendance record {attendance_record_id} not found")
    return _to_response(row)
...
def delete_attendance_record(attendance_record_id: int, db: Session | None = None) -> dict[str, str]:
    ...
    attendance_repo.delete_attendance_record(attendance_record_id, db)
