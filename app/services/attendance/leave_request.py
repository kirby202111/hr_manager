...
def _require_row(leave_request_id: int, db: Session | None = None) -> dict:
    row = attendance_repo.get_leave_request_by_id(leave_request_id, db)
    if row is None:
...
def list_leave_requests(
    ...
    rows = attendance_repo.list_leave_requests(worker_id, status, leave_type, start_date, end_date, db)
...
def create_leave_request(data: LeaveRequestCreate, db: Session | None = None) -> LeaveRequestResponse:
    ...
    row = attendance_repo.create_leave_request(payload, db)
...
def update_leave_request(
    ...
    _validate_payload(payload, db)
    row = attendance_repo.update_leave_request(leave_request_id, data.model_dump(exclude_unset=True), db)
    if row is None:
        raise NotFoundError(f"Leave request {leave_request_id} not found")
    return _to_response(row)
...
def delete_leave_request(leave_request_id: int, db: Session | None = None) -> dict[str, str]:
    ...
    attendance_repo.delete_leave_request(leave_request_id, db)
