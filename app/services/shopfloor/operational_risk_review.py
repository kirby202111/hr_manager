...
def _require_row(operational_risk_review_id: int, db: Session | None = None) -> dict:
    row = shopfloor_repo.get_operational_risk_review_by_id(operational_risk_review_id, db)
    if row is None:
...
def list_operational_risk_reviews(
    ...
    rows = shopfloor_repo.list_operational_risk_reviews(risk_signal_id, review_status, db)
...
def create_operational_risk_review(
    ...
    if shopfloor_repo.get_operational_risk_signal_by_id(data.risk_signal_id, db) is None:
        raise NotFoundError(f"Operational risk signal {data.risk_signal_id} not found")
    row = shopfloor_repo.create_operational_risk_review(data.model_dump(), db)
...
def update_operational_risk_review(
    ...
    if shopfloor_repo.get_operational_risk_signal_by_id(payload["risk_signal_id"], db) is None:
        raise NotFoundError(f"Operational risk signal {payload['risk_signal_id']} not found")
    row = shopfloor_repo.update_operational_risk_review(
...
def delete_operational_risk_review(operational_risk_review_id: int, db: Session | None = None) -> dict[str, str]:
    ...
    shopfloor_repo.delete_operational_risk_review(operational_risk_review_id, db)
