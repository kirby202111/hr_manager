...
def _require_row(project_skill_requirement_id: int, db: Session | None = None) -> dict:
    row = collaboration_repo.get_project_skill_requirement_by_id(project_skill_requirement_id, db)
    if row is None:
...
def list_project_skill_requirements(
    ...
    rows = collaboration_repo.list_project_skill_requirements(project_id, skill_id, db)
...
def create_project_skill_requirement(
    ...
    if (
        collaboration_repo.get_project_skill_requirement_by_project_and_skill(
            payload["project_id"], payload["skill_id"], db
        )
        is not None
    ):
        raise ConflictError("Project skill requirement already exists")
    row = collaboration_repo.create_project_skill_requirement(payload, db)
...
def update_project_skill_requirement(
    ...
    _validate_payload(payload, db)
    existing = collaboration_repo.get_project_skill_requirement_by_project_and_skill(
        payload["project_id"], payload["skill_id"], db
    )
    if existing is not None and existing["id"] != project_skill_requirement_id:
        raise ConflictError("Project skill requirement already exists")
    row = collaboration_repo.update_project_skill_requirement(
...
def delete_project_skill_requirement(project_skill_requirement_id: int, db: Session | None = None) -> dict[str, str]:
    ...
    collaboration_repo.delete_project_skill_requirement(project_skill_requirement_id, db)
