...
from app.models import performance as performance_model
...
def create_cycle(data: ReviewCycleCreate) -> ReviewCycleResponse:
    ...
    cycle = performance_model.create_cycle(cycle_data)
    return ReviewCycleResponse(review_count=0, **cycle)


def list_cycles() -> ReviewCycleListResponse:
    cycles = performance_model.get_all_cycles()
    result = []
    for c in cycles:
        reviews = performance_model.get_reviews_by_cycle(c["id"])
...
def get_cycle(cycle_id: int) -> ReviewCycleResponse:
    cycle = performance_model.get_cycle_by_id(cycle_id)
    if cycle is None:
        raise HTTPException(status_code=404, detail=f"Review cycle {cycle_id} not found")
    reviews = performance_model.get_reviews_by_cycle(cycle_id)
...
def update_cycle(cycle_id: int, data: ReviewCycleUpdate) -> ReviewCycleResponse:
    ...
    updated = performance_model.update_cycle(cycle_id, update_data)
    reviews = performance_model.get_reviews_by_cycle(cycle_id)
...
def _fill_names(record: dict) -> dict:
    ...
    cycle = performance_model.get_cycle_by_id(record["cycle_id"])
...
def create_review(data: PerformanceReviewCreate) -> PerformanceReviewResponse:
    ...
    cycle = performance_model.get_cycle_by_id(data.cycle_id)
    if cycle is None:
    ...
    existing = performance_model.get_review_by_employee_cycle(data.employee_id, data.cycle_id)
    ...
    record = performance_model.create_review(review_data)
...
def list_reviews(employee_id: int | None = None, cycle_id: int | None = None) -> PerformanceReviewListResponse:
    records = performance_model.get_all_reviews(employee_id, cycle_id)
...
def get_review(review_id: int) -> PerformanceReviewResponse:
    record = performance_model.get_review_by_id(review_id)
    if record is None:
...
def update_review(review_id: int, data: PerformanceReviewUpdate) -> PerformanceReviewResponse:
    ...
    updated = performance_model.update_review(review_id, update_data)
...
def get_employee_summary(employee_id: int) -> EmployeePerformanceSummary:
    ...
    reviews = performance_model.get_reviews_by_employee(employee_id)
    if not reviews:
