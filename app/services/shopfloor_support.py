...
from app.schemas.shopfloor import ListResponse
...
def employee_exists(employee_id: int, db: Session | None = None) -> None:
    if employee_repo.get_worker_by_id(employee_id, db) is None:
        raise ValidationError(f"Worker {employee_id} not found")
